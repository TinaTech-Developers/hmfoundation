    ...
    <div className="flex flex-col min-h-screen bg-gray-50 text-gray-900">
      ...
      <header className="sticky top-0 z-50 shadow-md bg-white px-6 py-4 flex justify-between items-center">
        ...
        <button
          ...
          className="flex bg-red-700 p-2 text-white items-center gap-2 rounded-lg hover:bg-green-800 transition"
        >
          ...
          <nav className="space-y-2">
            {navItems.map((item) => (
              <Link
                key={item.label}
                href={item.href}
                className="block px-4 py-2 rounded-lg hover:bg-green-200 transition font-medium text-green-800"
              >
                {item.label}
              </Link>
            ))}
